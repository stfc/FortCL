...
program vadd
  ...
  integer :: i, ierr, myrank, num_of_ranks

  ! Initialize MPI
  call MPI_INIT(ierr)
  call MPI_COMM_RANK(MPI_COMM_WORLD, myrank, ierr)
  call MPI_COMM_SIZE(MPI_COMM_WORLD, num_of_ranks, ierr)
  ...
  ! Initialise the OpenCL device and environment (serialized for nicer output)
  do i=0, num_of_ranks
    call MPI_BARRIER(MPI_COMM_WORLD, ierr)
    if (myrank == i) then
      write(*,*) "Initializing OpenCL context for MPI rank ", myrank
      call ocl_env_init(1, myrank + 1)
